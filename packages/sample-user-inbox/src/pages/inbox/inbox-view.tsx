...
import {
  ActitoUserInbox,
  ActitoUserInboxItem,
} from 'react-native-actito-user-inbox';
...
export const InboxView = () => {
  ...
  const [items, setItems] = useState<ActitoUserInboxItem[]>([]);
  const [selectedItem, setSelectedItem] = useState<ActitoUserInboxItem | null>(
    null
  );
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const userInboxResponse =
        await ActitoUserInbox.parseResponseFromString(requestResponseStr);
  ...
  useEffect(
    function setupListeners() {
      const subscriptions = [
        ActitoPush.onNotificationOpened(async (_) => {
          await refresh();
        }),

        ActitoPush.onNotificationInfoReceived(async (_) => {
          await refresh();
        }),
      ];
  ...
  async function open(item: ActitoUserInboxItem) {
    try {
      const notification = await ActitoUserInbox.open(item);
  ...
  async function markAsRead(item: ActitoUserInboxItem) {
    try {
  ...
  async function remove(item: ActitoUserInboxItem) {
    try {
