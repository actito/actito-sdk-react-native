...
const LINKING_ERROR =
  `The package 'react-native-actito-scannables' doesn't seem to be linked. Make sure: \n\n` +
...
const ActitoScannablesModule = isTurboModuleEnabled
  ? require('./NativeActitoScannablesModule').default
  : NativeModules.ActitoScannablesModule;

const NativeModule = ActitoScannablesModule
  ? ActitoScannablesModule
  : new Proxy(
      {},
      {
        get() {
...
export class ActitoScannables {
  ...
  public static onScannableDetected(
    callback: (scannable: ActitoScannable) => void
  ): EmitterSubscription {
