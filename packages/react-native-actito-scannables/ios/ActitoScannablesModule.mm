#import "ActitoScannablesModule.h"

#if __has_include(<react_native_actito_scannables/react_native_actito_scannables-Swift.h>)
#import <react_native_actito_scannables/react_native_actito_scannables-Swift.h>
#else
#import "react_native_actito_scannables-Swift.h"
#endif

@interface ActitoScannablesModule () <ActitoScannablesModuleDelegate>
@end

@implementation ActitoScannablesModule {
    ActitoScannablesPlugin *plugin;
}

- (instancetype)init {
    self = [super init];
    if(self) {
        plugin = [ActitoScannablesPlugin new];
...
{
    return std::make_shared<facebook::react::NativeActitoScannablesModuleSpecJSI>(params);
}
#endif
