...
const LINKING_ERROR =
  `The package 'react-native-actito-in-app-messaging' doesn't seem to be linked. Make sure: \n\n` +
...
const ActitoInAppMessagingModule = isTurboModuleEnabled
  ? require('./NativeActitoInAppMessagingModule').default
  : NativeModules.ActitoInAppMessagingModule;

const NativeModule = ActitoInAppMessagingModule
  ? ActitoInAppMessagingModule
  : new Proxy(
      {},
      {
        get() {
...
export class ActitoInAppMessaging {
  ...
  public static onActionExecuted(
    callback: (data: {
      ...
      readonly action: ActitoInAppMessageAction;
    }) => void
  ): EmitterSubscription {
    return this.eventEmitter.addListener(
