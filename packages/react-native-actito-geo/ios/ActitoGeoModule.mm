#import "ActitoGeoModule.h"

#if __has_include(<react_native_actito_geo/react_native_actito_geo-Swift.h>)
#import <react_native_actito_geo/react_native_actito_geo-Swift.h>
#else
#import "react_native_actito_geo-Swift.h"
#endif

@interface ActitoGeoModule () <ActitoGeoModuleDelegate>
@end

@implementation ActitoGeoModule {
    ActitoGeoPlugin *plugin;
}

- (instancetype)init {
    self = [super init];
    if(self) {
        plugin = [ActitoGeoPlugin new];
...
{
    return std::make_shared<facebook::react::NativeActitoGeoModuleSpecJSI>(params);
}
#endif
