import ActitoGeoKit
import ActitoKit
import React

private let DEFAULT_ERROR_CODE = "actito_error"

@objc public protocol ActitoGeoModuleDelegate {
    func broadcastEvent(name: String, body: Any?)
}

@MainActor
@objc(ActitoGeoPlugin)
public class ActitoGeoPlugin: NSObject {
    @objc public weak var delegate: ActitoGeoModuleDelegate? = nil

    private var hasListeners = false
    private var eventQueue = [(name: String, payload: Any?)]()

    override public init() {
        super.init()

        logger.hasDebugLoggingEnabled = Actito.shared.options?.debugLoggingEnabled ?? false

        Actito.shared.geo().delegate = self
    }

    @objc
    public func startObserving() {
        hasListeners = true

        if !eventQueue.isEmpty {
            logger.debug("Processing event queue with \(eventQueue.count) items.")
            eventQueue.forEach { delegate?.broadcastEvent(name: $0.name, body: $0.payload)}
            eventQueue.removeAll()
        }
    }

    @objc
    public func stopObserving() {
        hasListeners = false
    }

    @objc
    public func supportedEvents() -> [String] {
        return [
            "com.actito.geo.location_updated",
            "com.actito.geo.region_entered",
            "com.actito.geo.region_exited",
            "com.actito.geo.beacon_entered",
            "com.actito.geo.beacon_exited",
            "com.actito.geo.beacons_ranged",
            "com.actito.geo.visit",
            "com.actito.geo.heading_updated",
        ]
    }

    private func dispatchEvent(_ name: String, payload: Any?) {
        if hasListeners {
            delegate?.broadcastEvent(name: name, body: payload)
        } else {
            eventQueue.append((name: name, payload: payload))
        }
    }

    // MARK: - Actito Geo

    @objc
    public func hasLocationServicesEnabled(_ resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) -> Void {
        resolve(Actito.shared.geo().hasLocationServicesEnabled)
    }

    @objc
    public func hasBluetoothEnabled(_ resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) -> Void {
        resolve(Actito.shared.geo().hasBluetoothEnabled)
    }

    @objc
    public func getMonitoredRegions(_ resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) -> Void {
        do {
            let payload = try Actito.shared.geo().monitoredRegions.map { try $0.toJson() }
            resolve(payload)
        } catch {
            reject(DEFAULT_ERROR_CODE, error.localizedDescription, nil)
        }
    }

    @objc
    public func getEnteredRegions(_ resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) -> Void {
        do {
            let payload = try Actito.shared.geo().enteredRegions.map { try $0.toJson() }
            resolve(payload)
        } catch {
            reject(DEFAULT_ERROR_CODE, error.localizedDescription, nil)
        }
    }

    @objc
    public func enableLocationUpdates(_ resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) -> Void {
        Actito.shared.geo().enableLocationUpdates()
        resolve(nil)
    }

    @objc
    public func disableLocationUpdates(_ resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) -> Void {
        Actito.shared.geo().disableLocationUpdates()
        resolve(nil)
    }
}

extension ActitoGeoPlugin: ActitoGeoDelegate {
    public func actito(_ actitoGeo: ActitoGeo, didUpdateLocations locations: [ActitoLocation]) {
        guard let location = locations.first else { return }

        do {
            dispatchEvent("com.actito.geo.location_updated", payload: try location.toJson())
        } catch {
            logger.error("Failed to emit the com.actito.geo.location_updated event.", error: error)
        }
    }

    public func actito(_ actitoGeo: ActitoGeo, didEnter region: ActitoRegion) {
        do {
            dispatchEvent("com.actito.geo.region_entered", payload: try region.toJson())
        } catch {
            logger.error("Failed to emit the com.actito.geo.region_entered event.", error: error)
        }
    }

    public func actito(_ actitoGeo: ActitoGeo, didExit region: ActitoRegion) {
        do {
            dispatchEvent("com.actito.geo.region_exited", payload: try region.toJson())
        } catch {
            logger.error("Failed to emit the com.actito.geo.region_exited event.", error: error)
        }
    }

    public func actito(_ actitoGeo: ActitoGeo, didEnter beacon: ActitoBeacon) {
        do {
            dispatchEvent("com.actito.geo.beacon_entered", payload: try beacon.toJson())
        } catch {
            logger.error("Failed to emit the com.actito.geo.beacon_entered event.", error: error)
        }
    }

    public func actito(_ actitoGeo: ActitoGeo, didExit beacon: ActitoBeacon) {
        do {
            dispatchEvent("com.actito.geo.beacon_exited", payload: try beacon.toJson())
        } catch {
            logger.error("Failed to emit the com.actito.geo.beacon_exited event.", error: error)
        }
    }

    public func actito(_ actitoGeo: ActitoGeo, didRange beacons: [ActitoBeacon], in region: ActitoRegion) {
        do {
            let payload: [String: Any] = [
                "region": try region.toJson(),
                "beacons": try beacons.map { try $0.toJson() },
            ]

            dispatchEvent("com.actito.geo.beacons_ranged", payload: payload)
        } catch {
            logger.error("Failed to emit the com.actito.geo.beacons_ranged event.", error: error)
        }
    }

    public func actito(_ actitoGeo: ActitoGeo, didVisit visit: ActitoVisit) {
        do {
            dispatchEvent("com.actito.geo.visit", payload: try visit.toJson())
        } catch {
            logger.error("Failed to emit the com.actito.geo.visit event.", error: error)
        }
    }

    public func actito(_ actitoGeo: ActitoGeo, didUpdateHeading heading: ActitoHeading) {
        do {
            dispatchEvent("com.actito.geo.heading_updated", payload: try heading.toJson())
        } catch {
            logger.error("Failed to emit the com.actito.geo.heading_updated event.", error: error)
        }
    }
}
