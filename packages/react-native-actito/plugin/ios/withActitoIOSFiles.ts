...
const withSetActitoOptionsFile: ConfigPlugin<ActitoPluginProps> = (
  config,
  props
) => {
...
export const withActitoIOSFiles: ConfigPlugin<ActitoPluginProps> = (
  config,
  props
) => {
  config = withSetActitoServicesFile(config, props);
  config = withSetActitoOptionsFile(config, props);
