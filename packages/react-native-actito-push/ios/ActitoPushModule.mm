#import "ActitoPushModule.h"

#if __has_include(<react_native_actito_push/react_native_actito_push-Swift.h>)
#import <react_native_actito_push/react_native_actito_push-Swift.h>
#else
#import "react_native_actito_push-Swift.h"
#endif

@interface ActitoPushModule () <ActitoPushModuleDelegate>
@end

@implementation ActitoPushModule {
    ActitoPushPlugin *plugin;
}

- (instancetype)init {
    self = [super init];
    if(self) {
        plugin = [ActitoPushPlugin new];
...
{
    return std::make_shared<facebook::react::NativeActitoPushModuleSpecJSI>(params);
}
#endif
