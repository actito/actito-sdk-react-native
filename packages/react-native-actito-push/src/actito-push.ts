...
const ActitoPushModule = isTurboModuleEnabled
  ? require('./NativeActitoPushModule').default
  : NativeModules.ActitoPushModule;

const NativeModule = ActitoPushModule
  ? ActitoPushModule
  : new Proxy(
      {},
      {
...
export class ActitoPush {
  ...
  public static onSubscriptionChanged(
    callback: (subscription?: ActitoPushSubscription | null) => void
  ): EmitterSubscription {
    return this.eventEmitter.addListener(
