...
import { copyResources } from 'react-native-actito/lib/plugin';
...
export async function setupNotificationServiceExtension(
  ...
  const pluginDir = require.resolve('react-native-actito-push/package.json');

  const extFilesDir = path.join(
    pluginDir,
    `../lib/plugin/ios/${ACTITO_NOTIFICATION_SERVICE_EXTENSION}/`
  );

  const isExtExists = fs.existsSync(
    `${projRoot}/ios/${ACTITO_NOTIFICATION_SERVICE_EXTENSION}`
  );
  const isTargetExists = proj.pbxTargetByName(
    ACTITO_NOTIFICATION_SERVICE_EXTENSION
  );
