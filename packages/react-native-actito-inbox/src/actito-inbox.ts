...
const LINKING_ERROR =
  `The package 'react-native-actito-inbox' doesn't seem to be linked. Make sure: \n\n` +
...
const ActitoInboxModule = isTurboModuleEnabled
  ? require('./NativeActitoInboxModule').default
  : NativeModules.ActitoInboxModule;

const NativeModule = ActitoInboxModule
  ? ActitoInboxModule
  : new Proxy(
      {},
      {
...
export class ActitoInbox {
   ...
   * @return {Promise<ActitoNotification>} - A promise that resolves to the
   * {@link ActitoNotification} associated with the inbox item.
   */
  ...
  public static onInboxUpdated(
    callback: (items: ActitoInboxItem[]) => void
  ): EmitterSubscription {
