...
import { ActitoAsset, ActitoAssets } from 'react-native-actito-assets';
...
export default function AssetsScreen() {
  ...
  const [assets, setAssets] = useState<ActitoAsset[]>([]);

  async function fetchAssets() {
    setAssets([]);

    try {
      const result = await ActitoAssets.fetch(assetsGroup);
      setAssets(result);
      setAssetsGroup('');
